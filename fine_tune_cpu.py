...
# --- Configuration ---
MODEL_BASE = "gpt2"  # <-- Our new CPU-friendly model
MODEL_FINETUNE_DIR = "./gpt2-finetuned-python"
DATASET_NAME = "iamtarun/python_code_instructions_18k_alpaca"
# We don't specify a device. Trainer will default to CPU.
# ---------------------
...
def main():
    ...
    # We will only use a small subset of the data for a quick CPU fine-tune
    # 18,000 samples is too much for a CPU. Let's use 2,000.
    tokenized_dataset = dataset.map(
        tokenize_function, 
        batched=True, 
        remove_columns=dataset.column_names
    ).shuffle(seed=42).select(range(2000)) # <-- Select 2000 random samples
